...
# 文档信息
- **文件名**: 总框架
- **原始文件名**: 总框架_id_00000000_level_0_parentId_null_time_20251218_Creator_lvwan01
- **Level**: 0
- **ID**: 00000000
- **ParentID**: null
- **Time**: 20251218
...
> 
> *   **定义**: 指的是应用程序的两种运行形态。
>     *   **Native 模式 (桌面端)**: 由 `crates/app` (Tauri Host) 驱动，提供系统级窗口与极致性能。
>     *   **Web 模式 (浏览器端)**: 由 `crates/web-client` (WASM Host) 驱动，实现免安装的跨平台访问。
> *   **基石**: [[显示引擎.md]] (`crates/display`) 是双模共享的绘图后端，基于 `wgpu` 自动适配 Vulkan/Metal (Native) 或 WebGL/WebGPU (Web)。
> *   **文档**: 详见 [[双模架构.md]] 与 [[前端架构.md]]。
...
### 3.1 [[基础设施.md|基础设施层 (Infrastructure)]] - Level 1
*   **ID**: `10000000`
*   **功能**: 提供全局通用的基础服务。
*   **核心组件**:
    *   **日志系统**: 统一的结构化日志。
...
### 3.2 [[几何内核.md|几何内核 (Geometry Kernel)]] - Level 1
*   **ID**: `11000000`
...
### 3.3 [[显示引擎.md|显示引擎 (Display Engine)]] - Level 1
*   **ID**: `12000000`
...
### 3.4 [[交互系统.md|交互与输入系统 (Interaction & Input)]] - Level 1
*   **ID**: `13000000`
...
### 3.5 [[核心逻辑.md|核心逻辑层 (Kernel)]] - Level 1
*   **ID**: `02000000`
...
### 3.6 [[数据存储.md|数据模型与持久化 (Storage)]] - Level 1
*   **ID**: `14000000`
*   **功能**: 数据的序列化、反序列化与交换。
*   **核心组件**:
    *   **Format**: 自定义文件格式定义。
...
### 3.7 [[AI系统.md|AI 系统 (AI System)]] - Level 1
*   **ID**: `15000000`
*   **功能**: 智能化服务层。
*   **核心组件**:
...
### 3.8 [[前端架构.md|前端与桥接 (Web Client & Bridge)]] - Level 1
*   **ID**: `18000000` / `17000000`
*   **功能**: 用户界面与底层通信。
*   **核心组件**:
    *   **[[互操作协议.md|Bridge Protocol]]**: 定义 JSON 通信协议 (Command/Event)。
    *   **Overlay UI**: 透明叠加的 Vue 3 界面。
    *   **Tauri/WASM Adapter**: 适配不同宿主环境的通信层。

## 4. 扩展性设计 (Extensibility)
*   **[[脚本系统.md|脚本系统]] (ID: 16000000)**: 支持 Python/Lua 运行时，通过 FFI 调用 Kernel 接口。
