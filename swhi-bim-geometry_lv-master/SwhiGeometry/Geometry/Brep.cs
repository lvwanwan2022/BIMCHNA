...
{
  public class Brep : Base, IHasArea, IHasVolume
  {
    ...
    [DetachProperty]
    public List<MeshXYZ> displayValue { get; set; }
    ...
    public List<double> VerticesValue
    {
      ...
      set
      {
        if (value != null)
        {
          //var units = value.Count % 3 == 0 ? UnitsType.None : UnitsType.GetUnitFromEncoding(value[0]);
          for (int i = value.Count % 3 == 0 ? 0 : 1; i < value.Count; i += 3)
          {
            Vertices.Add(new XYZ(value[i], value[i + 1], value[i + 2]));
          }
        }
      }
    }
    ...
    public Brep(string provenance, MeshXYZ displayValue)
      : this(provenance, new List<MeshXYZ>{displayValue})
    { }
    
    public Brep(string provenance, List<MeshXYZ> displayValues) : this()
    {
    ...
    /// <returns></returns>
    public bool TransformTo(TransformXYZ transform, out Brep brep)
    {
      var displayValues = new List<MeshXYZ>(displayValue.Count);
      foreach (MeshXYZ v in displayValue)
      {
        v.TransformTo(transform, out MeshXYZ mesh);
    ...
    public MeshXYZ displayMesh {
      get => displayValue?.FirstOrDefault();
      set => displayValue = new List<MeshXYZ> {value};
    }
    #endregion
  }
  ...
  public class BrepEdge : Base
  {
    ...
    [JsonIgnore] public XYZ StartVertex => Brep.Vertices[StartIndex];
    [JsonIgnore] public XYZ EndVertex => Brep.Vertices[EndIndex];
