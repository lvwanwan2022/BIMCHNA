...
namespace Swhi.BIM.Geometry
{
  ...
  public class BaseObjectSerializer
  {
    ...
    /// <summary>
    /// 用于描述对象在Swhi.BIM命名空间中的type
    /// </summary>
    public string TypeDiscriminator = "swhiGeometry_type";
    ...
    public string Serialize(Base baseObj)
    {
      ...
      finally
      {
        ...
        // `Preserialize`意味着将所有对象转换为最终形式，最终形式将显示在json中，.net对象
    ...
    {
      bool computeClosuresForChild = (detachInfo.IsDetachable || detachInfo.IsChunkable);
